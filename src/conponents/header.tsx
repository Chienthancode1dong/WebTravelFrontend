        ...
        <div className="hidden sm:flex gap-6 items-center text-[18px]">
          {isAuthenticated && user ? (
            <div className="flex items-center gap-2">
              <span>Welcome, {user.email}</span> {/* user.email actually contains name */}
              <button
                onClick={logoutUser}
            ...
            <>
              <Link 
        href={'/login'} 
        className="bg-orange-500 hover:bg-orange-600 px-3 py-2 rounded-4xl cursor-pointer select-none transition-colors inline-block text-center"
      >
        Login
      </Link>
            </>
