                ...
                {bookings.map((booking) => (
                    <div key={booking._id} className='grid grid-cols-1 md:grid-cols-[4fr_3fr_2fr] w-full border-b border-gray-300 py-6 first:border-t'>
                        ...
                        <div className='flex flex-col md:flex-row'>
                            <Image src={booking.room.images[0]} alt="hotel-img" className='min-md:w-44 rounded shadow object-cover' width={176} height={120} />
                            <div className='flex flex-col gap-1.5 max-md:mt-3 min-md:ml-4'>
                                <p className='font-playfair text-2xl'>{booking.hotel.name}
                                    <span className='font-inter text-sm'> ({booking.room.roomType})</span>
                                </p>
                                <div className='flex items-center gap-1 text-sm text-gray-500'>
                                    <Image src={assets.locationIcon} alt="location-icon" width={18} height={18} />
                                    <span>{booking.hotel.address}</span>
                                </div>
                                <div className='flex items-center gap-1 text-sm text-gray-500'>
                                    <Image src={assets.guestsIcon} alt="guests-icon" width={18} height={18} />
